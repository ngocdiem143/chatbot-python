        ...
        if data['object'] == "page":
			...
			# using a GET request to extract the user's information. In the below lines of code, we get the name of the user. Now, we can call the user by his name.
                ...
                if messaging_event.get('postback'):
                    ...
                    if messaging_event['postback'].get('payload') == '___ asked product1 details':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": detail1}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                        
                     
                    elif messaging_event['postback'].get('payload') == '___ asked product1 image':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": "ảnh của sản phẩm"}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                        com.send_img(sender_id,product1_image)
                        
                      
                    elif messaging_event['postback'].get('payload') == '___ asked order':
                        
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": order}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                        

                    elif messaging_event['postback'].get('payload') == '___ asked product2 details':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": detail2}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                        
                        
                    elif messaging_event['postback'].get('payload') == '___ asked product2 image':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": "ảnh của sản phẩm"}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                    ...
                    elif messaging_event['postback'].get('payload') == '___ asked product3 details':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": detail3}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                        
                      
                    elif messaging_event['postback'].get('payload') == '___ asked product3 image':
                    ...
                    elif messaging_event['postback'].get('payload') == '___ asked product4 details':
                        
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": detail4}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                        
                        
                    elif messaging_event['postback'].get('payload') == '___ asked product4 image':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": "ảnh của sản phẩm"}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                    ...
                    elif messaging_event['postback'].get('payload') == '___ asked product5 details':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": detail5}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                        
                      
                    elif messaging_event['postback'].get('payload') == '___ asked product5 image':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": "ảnh của sản phẩm"}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                    ...
                    elif messaging_event['postback'].get('payload') == '___ asked product6 details':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": detail6}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                        
                        
                    elif messaging_event['postback'].get('payload') == '___ asked product6 image':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": "ảnh của sản phẩm"}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                    ...
                    elif messaging_event['postback'].get('payload') == '___ asked product7 details':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": detail7}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                        
                      
                    elif messaging_event['postback'].get('payload') == '___ asked product7 image':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": "ảnh của sản phẩm"}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                    ...
                    elif messaging_event['postback'].get('payload') == '___ asked product8 details':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": detail8}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                     
                    elif messaging_event['postback'].get('payload') == '___ asked product8 image':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": "ảnh của sản phẩm"}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
                    ...
                    if messaging_event['postback'].get('payload') == 'tham_quan':
                        response = requests.post(fb_api,params=token_dict, json={"message": {"text": link_shopee}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
...
#===============================================================  
                    ...
                    if messaging_event['message'].get('quick_reply'):
                        ...
                        if messaging_event['message']['quick_reply'].get('payload') == '___ clicked generic continue':
                            gen_carousel(sender_id)
                        if messaging_event['message']['quick_reply'].get('payload') == 'tham_quan':
                            response = requests.post(fb_api,params=token_dict, json={"message": {"text": link_shopee}, "recipient": {"id": sender_id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})			
...
def gen_carousel(id):
    ...
    response = requests.post(fb_api,params=token_dict, json={"message": {"text": "Tiếp tục\nHãy tham khảo các sản phẩm của chúng tôi"}, "recipient": {"id": id}, "notification_type": "REGULAR", "messaging_type": "RESPONSE"})
    response2 = requests.post(fb_api,params=token_dict, json=carousel_json)

def gen_continue_button(id):
	response2 = requests.post(fb_api,params=token_dict, json={"recipient":{"id": id}, "messaging_type": "RESPONSE","message":{"text": "Bấm Continue để tiếp tục\n👇👇👇","quick_replies":[{"content_type":"text","title":"Continue","payload":"___ clicked generic continue"}]}})
def gen_link_shopee_button(id):
	response2 = requests.post(fb_api,params=token_dict, json={"recipient":{"id": id}, "messaging_type": "RESPONSE","message":{"text": "Ghé thăm gian hàng shopee của chúng tôi:","quick_replies":[{"content_type":"text","title":"Ghé thăm","payload":"tham_quan"}]}})
...
def handleMessage(senderPsid, receivedMessage):
    ...
    if 'text' in receivedMessage:
        ...
        if toSend.lower() in  greeting :
            com.send_message(sender_id, "Hi {} {}, Chào mừng bạn ghé thăm Python Shop.Hãy gửi cho chúng tôi bất kỳ câu hỏi nào của bạn!!".format(l_name, f_name))
            
            com.send_message(sender_id, "Hãy tham khảo một số sản phẩm của shop:")
            com.send_img(sender_id,'https://cf.shopee.vn/file/943849ee53628608fded6e0fe5b15c74')
            com.send_img(sender_id, 'https://cf.shopee.vn/file/d6d2f6f5afd89c612fc980b481655da4')
            com.send_img(sender_id, 'https://cf.shopee.vn/file/2f6bf9ae032e6cdddb62b78499535e53')

        elif toSend == "send image":
            com.send_message(
                sender_id, "Đây là ảnh đại diện của bạn:")
        ...
        response = {"text": 'Xin lỗi bạn nhưng chúng tôi chỉ chấp nhận tin nhắn text. Các tệp đính kèm sẽ không được cháp nhận. Hãy thử lại!'}
        callSendAPI(senderPsid, response)
